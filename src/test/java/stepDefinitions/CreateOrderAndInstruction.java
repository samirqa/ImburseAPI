...
public class CreateOrderAndInstruction {
        ...
        @Given("with the endpoint {string} generate the AccessToken")
        public void generateAccessToken(String p) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        		configFileReader = new ConfigFileReader();
                RestAssured.baseURI = configFileReader.getBaseUrl();
                		//"https://sandbox-api.imbursepayments.com/v1";
                path = p;
        ...
        public void sendValidOrderReference() {
                ...
                response = given().contentType(ContentType.JSON).header("Authorization", "bearer " + accessToken)
                                .header("x-account-id", configFileReader.getAccountID()).header("x-tenant-id", configFileReader.getTenantID())
                                .body("{\"orderRef\": \"{{name}}\", \"metadata\": { \"key1\": \"a\",\"key2\": \"b\",\"key3\": \"c\"}}"
                                                .replace("{{name}}", name))
                                .post(path).then().extract().response();
        }

        @When("send order reference name {string}")
    ...
    public void checkResponseError(String expectedErrorMsg) {
    	String actualErrorMsg = response.jsonPath().getString("errors");
    	Assertions.assertTrue(actualErrorMsg.contains(expectedErrorMsg));
    }
